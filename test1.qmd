...
rpart.plot(model_cart$fit, roundint = FALSE, cex = 0.8,
           main = "Decision Tree: Return Prediction", box.palette = "Blues")
```
...
cart_pred <- predict(model_cart, test_baked) %>% bind_cols(truth = test_baked$return_next)

lm_metrics <- lm_pred %>% yardstick::metrics(truth = truth, estimate = .pred)
cart_metrics <- cart_pred %>% yardstick::metrics(truth = truth, estimate = .pred)
...
card_6_data_actual <- card_6_data 
  # filter(DataType == "Actual Sale")  # ← KEY FILTER

cat("\n=== After Filtering to Actual Sales ===\n")
cat("Actual sale observations:", nrow(card_6_data_actual), "\n")
cat("Removed interpolated:", nrow(card_6_data) - nrow(card_6_data_actual), "observations\n")

cat("=== CARD 6 INFO ===\n")
cat("Card Name:", unique(card_6_data$card_name)[1], "\n")
cat("Set:", unique(card_6_data$set_name)[1], "\n")
cat("Observations:", nrow(card_6_data), "\n")
```

## Feature Engineering

```{r card6-features}
#| label: card6-features

card_6_features <- card_6_data_actual %>%
  left_join(sentiment_daily, by = "Date") %>%
  left_join(spy_daily, by = "Date") %>%
  mutate(
...
cat("Card 6 features:", nrow(card_6_features), "observations\n")
```
...
card6_split <- rsample::initial_time_split(card_6_features, prop = 0.70)
card6_train <- rsample::training(card6_split)
card6_test <- rsample::testing(card6_split)

cat("Card 6 - Train:", nrow(card6_train), "| Test:", nrow(card6_test), "\n")

card6_recipe <- recipes::recipe(
...
card6_test_baked <- recipes::bake(card6_recipe, card6_test)
```
...
card6_pred <- predict(model_lm, card6_test_baked) %>%
  bind_cols(truth = card6_test_baked$return_next)

card6_metrics <- card6_pred %>%
  yardstick::metrics(truth = truth, estimate = .pred)

card6_metrics %>%
  gt() %>%
  fmt_number(columns = .estimate, decimals = 4) %>%
  tab_header(title = "Card 6: Out-of-Sample Performance")

card6_rsq <- card6_metrics %>% filter(.metric == "rsq") %>% pull(.estimate)
basket_rsq <- lm_metrics %>% filter(.metric == "rsq") %>% pull(.estimate)

cat("\n=== COMPARISON ===\n")
cat("Card 6 R²:  ", round(card6_rsq * 100, 2), "%\n")
...
card6_trades <- card6_pred %>%
  bind_cols(card_6_features %>% slice_tail(n = nrow(card6_test)) %>% select(Date, Price)) %>%
  mutate(
    signal = case_when(
      .pred > 0.01 ~ "BUY",
      .pred < -0.01 ~ "SELL",
...
strategy_performance <- card6_trades %>%
  summarise(
...
ggplot(card6_trades, aes(x = Date)) +
  ...
  labs(
    title = paste("Card 6:", unique(card_6_data$card_name)[1]),
...
ggplot(card7_trades, aes(x = Date)) +
  ...
  scale_color_manual(values = c("ML Strategy" = "#e74c3c", "Buy & Hold" = "#95a5a6")) +
  labs(
    title = paste("Card 7:", unique(card_6_data$card_name)[1]),
    subtitle = "ML Strategy vs Buy-and-Hold",
    x = "Date", y = "Cumulative Return"
  ) +
  theme_minimal() +
